...
2. **Named Recipient Only:** This grant is currently issued solely to the OpenClaw project. The Licensor reserves the right to issue similar non-exclusive grants to other entities at their discretion.
3. **Patent Notice Requirement:** Any OpenClaw implementation must prominently display: *"PROTECTED BY PATENT PENDING (Priority Established 2026) Implementation permitted via non-exclusive grant to OpenClaw." and link back to the segregation_engine_core GitHub page (https://github.com/aiguysolutions/segregation_engine_core). *
